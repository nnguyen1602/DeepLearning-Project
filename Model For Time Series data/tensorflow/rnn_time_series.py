...
lstm_layer1 = tf.nn.rnn_cell.BasicLSTMCell(input_dim*3)
...
######LSTM layer is followed by a simple linear regression layer
# whose output becomes the final output
##The Regression-Output lstm_layer1
...
## Input for correct_output (for training)
correct_output = tf.placeholder(tf.float32, [1, input_dim])
##Calculate the sum of squares Error
error = tf.pow(tf.sub(final_ouput, correct_output), 2)
## The Optimizer
#Adam works best
...
for i in range(80000):
    ...
    _, _, network_output = sess.run([lstm_update_op1, train_step,final_ouput], feed_dict = {input_layer: input_v, correct_output: output_v})

    actual_output1.append(output_v[0][0])
    actualoutput2.append(output_v[0][1])
